...
"D1: Trig Integrals": (
    "You are an AI Question Generator for a Calculus I final review.\n"
    "Original exam model:\n"
    "D1(c) จงหา $\\int \\sin^4(x/3) dx$\n"
    "D1(a) จงหา $\\int \\sin^5 x \\cos^4 x \\, dx$\n\n"
    "Your task:\n"
    "1) Generate ONE NEW trigonometric integral problem that is 'ถึกขึ้น' (more tedious) than the models.\n"
    ...
    "   - To make it 'ถึก', the problem must require **nested applications** of this formula. For example, $\\int \\sin^6(x) \\, dx$ or $\\int \\sin^4(x) \\cos^2(x) \\, dx$. The expansion will create new $\\cos^2(...)$ or $\\cos^3(...)$ terms that need further reduction/integration.\n"
    ...
    "Output format:\n"
    "{\n"
    "  \"question_latex\": \"\\\\text{จงหาค่าของ } \\\\int ... dx\",\n"
    "  \"solution_latex\": \"\\\\begin{align*} ... \\\\end{align*}\"\n"
    "}\n"
    "Return ONLY this JSON object. No extra keys, no explanations outside LaTeX."
    ...
    "You are an AI Question Generator for a Calculus I final review.\n"
    "Original exam model:\n"
    "D2(b) จงหา $\\int e^{x}(ln~e^{x})^{2}dx$ (simplifies to $\\int x^2 e^x dx$)\n"
    "D2(d) จงหา $\\int x^{2}sin~x~dx$\n\n"
    "Your task:\n"
    "1) Generate ONE NEW integral problem that follows the most complex logic from D2: **repeated Integration by Parts (IBP)**.\n"
    "2) The problem must be 'ถึกขึ้น' (more tedious). It must require at least **two applications of IBP**.\n"
    ...
    "You are an AI Question Generator for a Calculus I final review.\n"
    "Original exam model:\n"
    "D3(a) ให้ $\\int_{0}^{3}(2x+1)f''(x) dx=10$ และ $f'(0)=f'(3)=1$ แล้ว จงหา $f(3)-f(0)$\n"
    "D3(b) ให้ $f(1)=2, f(4)=7, f'(1)=5, f'(4)=3$ ... จงหาค่าของ $\\int_{1}^{4}xf''(x)dx$\n\n"
    "Your task:\n"
    ...
    "2) The problem must be 'ถึกขึ้น' (significantly more tedious) by requiring **TWO applications of IBP**.\n"
    ...
    "Output format:\n"
    "{\n"
    "  \"question_latex\": \"\\\\text{กำหนดให้ } \\\\int_a^b ... = ... \\\\text{ และ } f(a)=..., f(b)=..., f'(a)=..., f'(b)=... \\\\text{ จงหาค่าของ } \\\\int_a^b f(x) dx\",\n"
    "  \"solution_latex\": \"\\\\begin{align*} ... \\\\end{align*}\"\n"
    "}\n"
    "Return ONLY this JSON object. No extra keys, no explanations outside LaTeX."
    ...
    "2) The problem must be 'ถึกขึ้น' (more tedious) by creating a new Region R (bounded by $x=f(y)$, $x=g(y)$, and the Y-axis) that **MUST be split into two parts** when integrating with respect to $y$.\n"
    ...
    "Output format:\n"
    "{\n"
    "  \"question_latex\": \"\\\\text{ให้ R เป็นบริเวณที่ปิดล้อมด้วย ... จงหาปริมาตร ... โดยวิธีเปลือกทรงกระบอก}\",\n"
    "  \"solution_latex\": \"\\\\begin{align*} ... \\\\end{align*}\"\n"
    "}\n"
    "Return ONLY this JSON object. No extra keys, no explanations outside LaTeX."
    ...
    "3) Make the problem 'ถึกขึ้น' (more tedious) than the originals:\n"
    ...
    "2) The problem must be 'ถึกขึ้น' (significantly more tedious).\n"
    ...
    "2) The problem must be 'ถึกขึ้น' (more tedious) by using a **different function**.\n"
    ...
    "Output format:\n"
    "{\n"
    "  \"question_latex\": \"\\\\text{กำหนดให้ } I_n = ... \\\\text{ (a) จงแสดงความสัมพันธ์ลดทอน } I_n = ... \\\\text{ (b) จงใช้ผลลัพธ์นี้หาค่า } I_...\",\n"
    "  \"solution_latex\": \"\\\\begin{align*} ... \\\\end{align*}\"\n"
    "}\n"
    "Return ONLY this JSON object. No extra keys, no explanations outside LaTeX."
    ...
    "You are an AI Question Generator for a Calculus I final review.\n"
    "Original exam model:\n"
    "E4(c) จงหาพื้นที่ของบริเวณที่อยู่ภายในวงกลม $x^{2}+y^{2}=12$ และวงกลม $(x-3)^{2}+(y-3)^{2}=3(\\sqrt{3}-1)^{2}$\n\n"
    "Your task:\n"
    "1) Generate ONE NEW area problem that follows the **core logic of E4(c): finding the area of a complex intersecting region**.\n"
    "2) The problem must be 'ถึกขึ้น' (significantly more tedious) by using **Polar Coordinates** instead of Cartesian coordinates.\n"
    ...
    "Output format:\n"
    "{\n"
    "  \"question_latex\": \"\\\\text{จงหาพื้นที่ของบริเวณที่ (ในพิกัดเชิงขั้ว) ... (เช่น อยู่ภายใน } r=... \\\\text{ และอยู่ภายนอก } r=...)\",\n"
    "  \"solution_latex\": \"\\\\begin{align*} ... \\\\end{align*}\"\n"
    "}\n"
    "Return ONLY this JSON object. No extra keys, no explanations outside LaTeX."
    ...
    "Your task:\n"
    "1) Generate ONE NEW integral problem that is 'ถึกขึ้น' (more tedious) by **combining all complex logic from F1**.\n"
    ...
    "You are an AI Question Generator for a Calculus I final review.\n"
    "Original exam model:\n"
    "F2. จงพิจารณาว่าอินทิกรัลไม่ตรงแบบต่อไปนี้ลู่เข้าหรือลู่ออก ถ้าลู่เข้า จงหาค่า\n"
    ...
    "2) The problem must be 'ถึกขึ้น' (more tedious) by making the integrand non-trivial, e.g., requiring a **u-substitution** (like $x e^{-x^2}$) or simple **Integration by Parts** to solve the antiderivative before evaluating the limits.\n"
    ...
    "Output format:\n"
    "{\n"
    "  \"question_latex\": \"\\\\text{จงพิจารณาว่าอินทิกรัลไม่ตรงแบบ ... ลู่เข้าหรือลู่ออก ถ้าลู่เข้า จงหาค่า}\",\n"
    "  \"solution_latex\": \"\\\\begin{align*} ... \\\\end{align*}\"\n"
"}\n"
    "Return ONLY this JSON object. No extra keys, no explanations outside LaTeX."
),"F3: Arc Length": (
    "You are an AI Question Generator for a Calculus I final review.\n"
    "Original exam model:\n"
    "F3. จงหาความยาวส่วนโค้งของ $y=\\frac{4e^{2x}+1}{4e^{x}}$ (which is $y = e^x + \\frac{1}{4}e^{-x}$) จาก $x=0$ ถึง $x=1$.\n"
    ...
    "2) The problem must be 'ถึกขึ้น' (more tedious) by using a **non-exponential function**, such as a polynomial $f(x) = ax^n + bx^{-m}$.\n"
    ...
    "Output format:\n"
    "{\n"
    "  \"question_latex\": \"\\\\text{จงหาความยาวของเส้นโค้ง } y = ... \\\\text{ จาก } x=a \\\\text{ ถึง } x=b\",\n"
    "  \"solution_latex\": \"\\\\begin{align*} ... \\\\end{align*}\"\n"
    "}\n"
    "Return ONLY this JSON object. No extra keys, no explanations outside LaTeX."
)
